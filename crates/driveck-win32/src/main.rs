...
mod app {
    ...
    use windows::{
        Win32::{
            ...
            Graphics::Gdi::{
                ...
                CreateFontIndirectW, CreateSolidBrush, DEFAULT_GUI_FONT, DRAW_TEXT_FORMAT,
                ...
                GetStockObject, HDC, HGDIOBJ, InvalidateRect, IntersectClipRect, PAINTSTRUCT,
                RDW_ALLCHILDREN, RDW_ERASE, RDW_INVALIDATE, RDW_UPDATENOW, RedrawWindow,
                RestoreDC, SRCCOPY, SaveDC, ScreenToClient, SelectObject, SetBkMode,
            ...
            UI::{
                ...
                WindowsAndMessaging::{
                    ...
                    CB_RESETCONTENT, CB_SETCURSEL, CBN_SELCHANGE, CBS_DROPDOWNLIST,
                    CREATESTRUCTW, CW_USEDEFAULT, CreateWindowExW, DefWindowProcW,
                    DestroyWindow, DispatchMessageW, ES_AUTOVSCROLL, ES_LEFT, ES_MULTILINE,
                    ES_READONLY, ES_WANTRETURN, GWLP_USERDATA, GetClientRect, GetMessageW,
                    GetSystemMetrics, GetWindowLongPtrW, GetWindowRect, HMENU, IDC_ARROW,
                    IDOK, KillTimer, LoadCursorW,
                    GetScrollInfo, MB_ICONERROR, MB_ICONWARNING, MB_OK, MB_OKCANCEL,
                    MESSAGEBOX_STYLE, MINMAXINFO, MSG, MessageBoxW, MoveWindow,
                    NONCLIENTMETRICSW, PM_REMOVE, PeekMessageW, PostMessageW,
                    PostQuitMessage, RegisterClassW,
                    SB_BOTTOM, SB_CTL, SB_LINEDOWN, SB_LINEUP, SB_PAGEDOWN, SB_PAGEUP,
                    SB_THUMBPOSITION, SB_THUMBTRACK, SB_TOP, SBM_SETSCROLLINFO, SBS_VERT,
                    SCROLLINFO, SIF_ALL, SIF_PAGE, SIF_POS, SIF_RANGE,
                    SM_CXSCREEN, SM_CYSCREEN,
                    SPI_GETNONCLIENTMETRICS, SW_HIDE, SW_SHOW, SW_SHOWNORMAL, SWP_NOACTIVATE,
                    SWP_NOZORDER, SendMessageW, SetForegroundWindow, SetTimer,
                    ...
                    WM_APP, WM_CLOSE, WM_COMMAND, WM_COPY, WM_CREATE, WM_DESTROY,
                    WM_DPICHANGED, WM_ERASEBKGND, WM_GETMINMAXINFO, WM_MOUSEWHEEL,
                    WM_NCCREATE, WM_PAINT,
                    WM_SETFONT, WM_SIZE, WM_TIMER, WM_VSCROLL, WNDCLASSW, WS_BORDER, WS_CHILD,
                    WS_CLIPCHILDREN, WS_EX_CLIENTEDGE, WS_OVERLAPPEDWINDOW, WS_TABSTOP,
        ...
        match message {
            ...
            WM_DRIVECK_PROGRESS => {
                ...
                if let Some(state) = state_mut(hwnd) {
                    ...
                    let basis_points = progress_basis_points(state.progress_current, state.progress_total);
            ...
            WM_DRIVECK_FINISHED => {
                ...
                if let Some(state) = state_mut(hwnd) {
                    ...
                    state.last_report_target_path = payload.report_text.as_ref().map(|_| payload.target_path.clone());
                    ...
                    if let Some(response) = payload.response.as_ref() {
                        ...
                        state.last_elapsed_text = state.last_elapsed_text.take().or_else(|| {
    ...
    unsafe extern "system" fn report_window_proc(
        hwnd: HWND,
        message: u32,
        wparam: WPARAM,
        lparam: LPARAM,
                        ...
                        IDC_REPORT_SAVE => {
                            if state.report_ready {
                                match save_report_text(hwnd, &state.report_text) {
                                    Ok(()) => {}
                                    Err(error) => {
                                        show_message(hwnd, "Failed to save report.", &error, MB_ICONERROR);
                                    }
                                }
                            }
                        }
    ...
    unsafe fn refresh_devices(state: &mut AppState) {
        ...
        if state
            .last_report_target_path
            .as_ref()
            .is_some_and(|path| Some(path.as_str()) != state.selected_target().map(|target| target.path.as_str()))
        {
    ...
    unsafe fn open_report_window(state: &mut AppState) {
    ...
    unsafe fn open_about_window(state: &mut AppState) {
    ...
    unsafe fn sync_report_window_from_main_state(state: &AppState) {
    ...
    unsafe fn update_actions(state: &mut AppState) {
        ...
        enable(state.device_combo, !busy && !state.device_targets.is_empty());
    ...
    unsafe fn update_idle_status(state: &mut AppState) {
        ...
        match state.selected_target() {
            Some(target) if target.is_mounted => {
                state.set_status("Will dismount", Tone::Warning)
            }
    ...
    unsafe fn handle_report_mouse_wheel(
        state: &mut AppState,
        wparam: WPARAM,
        lparam: LPARAM,
    ) -> bool {
    ...
    fn save_report_text(hwnd: HWND, report_text: &str) -> Result<(), String> {
        ...
        fs::write(&path, report_text).map_err(|error| {
            format!(
    ...
    unsafe fn paint_live_summary(hdc: HDC, state: &AppState, content: RECT) {
        ...
        draw_divider(hdc, content.left, content.right, row_top - scale_for_window(state.hwnd, 8));
        draw_metric_row(
            hdc,
            make_rect(content.left, row_top, rect_width(content), row_height),
            "Elapsed time",
    ...
    unsafe fn paint_result_summary(
        ...
        for (label, value) in [
            ...
            ("Validated size", format_bytes(report.validated_drive_size_bytes)),
            ("Highest valid region", format_bytes(report.highest_valid_region_bytes)),
    ...
    unsafe fn paint_report_window(hwnd: HWND, state: &ReportWindowState) {
        ...
        let title_rect = make_rect(banner_inner.left, banner_inner.top, rect_width(banner_inner), 18);
        draw_text_block(
            back_dc,
            title_rect,
            title,
    ...
    unsafe fn paint_about_window(hwnd: HWND, state: &AboutWindowState) {
        ...
        let desc_rect = make_rect(content.left, title_rect.bottom + 12, rect_width(content), 40);
        draw_text_block(
            back_dc,
            desc_rect,
    ...
    unsafe fn draw_validation_map(hdc: HDC, map_bounds: &RECT, grid_state: &ValidationGridState) {
        ...
        let padding = 8;
        let available_width =
            rect_width(*map_bounds) - gap * (GRID_COLUMNS as i32 - 1) - padding * 2;
        let available_height =
            rect_height(*map_bounds) - gap * (GRID_ROWS as i32 - 1) - padding * 2;
    ...
    unsafe fn draw_legend_item(
        ...
        let text_rect = make_rect(rect.left + 24, rect.top, rect_width(rect) - 24, rect_height(rect));
        draw_text_block(
            hdc,
            text_rect,
            label,
    ...
    unsafe fn draw_inline_status(
        ...
        let text_rect = make_rect(rect.left + 20, rect.top, rect_width(rect) - 20, rect_height(rect));
        draw_text_block(
            hdc,
            text_rect,
            text,
    ...
    unsafe fn draw_metric_multiline(
        hdc: HDC,
        rect: RECT,
        key: &str,
        value: &str,
    ...
    fn current_layout(hwnd: HWND) -> Layout {
        ...
        let progress_width =
            ((rect_width(footer_panel) * 30) / 100).clamp(scale_for_window(hwnd, 220), scale_for_window(hwnd, 360));
    ...
    fn current_report_layout(hwnd: HWND) -> ReportLayout {
        let client = current_client_rect(hwnd);
        let width = rect_width(client);
        let height = rect_height(client);
        ...
        let header_panel = make_rect(margin, margin, width - margin * 2, scale_for_window(hwnd, 112));
        ...
        let close_button = make_rect(width - margin - button_width, footer_y, button_width, button_height);
        let save_button = make_rect(close_button.left - gap - button_width, footer_y, button_width, button_height);
        let copy_button = make_rect(save_button.left - gap - button_width, footer_y, button_width, button_height);
    ...
    fn current_about_layout(hwnd: HWND) -> AboutLayout {
        let client = current_client_rect(hwnd);
        let width = rect_width(client);
        let height = rect_height(client);
    ...
    fn split_four(rect: RECT, gap: i32) -> [RECT; 4] {
    ...
    unsafe fn load_system_ui_font() -> (HGDIOBJ, bool) {
    ...
    fn dpi_for_window(hwnd: HWND) -> i32 {
    ...
    unsafe fn redraw_window_now(hwnd: HWND) {
        let _ = RedrawWindow(
            Some(hwnd),
            None,
            None,
    ...
    unsafe fn apply_suggested_dpi_rect(hwnd: HWND, lparam: LPARAM) {
    ...
    fn device_status_label(state: &AppState, target: &TargetInfo) -> String {
        ...
        } else if state.last_response.as_ref().is_some_and(|response| response.target.path == target.path) {
            "Finished".to_string()
    ...
    unsafe fn report_state_mut(hwnd: HWND) -> Option<&'static mut ReportWindowState> {
        ...
        if value == 0 {
            ...
            Some(&mut *(value as *mut ReportWindowState))
        }
    }
    ...
    unsafe fn take_report_state(hwnd: HWND) -> Option<*mut ReportWindowState> {
        ...
        if value == 0 {
            ...
            SetWindowLongPtrW(hwnd, GWLP_USERDATA, 0);
            Some(value as *mut ReportWindowState)
        }
    }
    ...
    unsafe fn about_state_mut(hwnd: HWND) -> Option<&'static mut AboutWindowState> {
        ...
        if value == 0 {
            ...
            Some(&mut *(value as *mut AboutWindowState))
        }
    }
    ...
    unsafe fn take_about_state(hwnd: HWND) -> Option<*mut AboutWindowState> {
        ...
        if value == 0 {
            ...
            SetWindowLongPtrW(hwnd, GWLP_USERDATA, 0);
            Some(value as *mut AboutWindowState)
        }
    }
